...
Signer.sign = function(opts, principal, privatekey) {
	...
	if(signer.opts.method !== 'GET') {
		if(signer.opts.json) {
			signer.opts.headers['content-md5'] = crypto.createHash('md5').update(JSON.stringify(signer.opts.body)).digest("base64")
		} else {
			signer.opts.headers['content-md5'] = crypto.createHash('md5').update(signer.opts.body).digest("base64")
		}
	}
